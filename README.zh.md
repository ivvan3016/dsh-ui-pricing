...
当组合提供 composer dock 时，本包注册一个 **cost** 占用项：显示当前会话的已计价费用（来自 `cost` projection）、当前本地时间与生效倍率，以及一条按倍率分档着色（折扣 <1、溢价 >1、中性 =1）的 24 小时条，带当前分钟标记。段中的北京时钟会平移到浏览器时区，使条跟随本地时钟。
